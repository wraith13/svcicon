...
public:
    ...
    DWORD start()
    {
        ...
        (
            ...
            (
                ...
                (
                    NULL,
                    L"このサービスはスタートアップの種類が\"無効\"に設定されています。\nこのサービスのスタートアップの種類を\"手動\"に変更してもいいですか？",
                    application_name,
                    MB_ICONEXCLAMATION |MB_OKCANCEL
                ) &&
                ChangeServiceConfig // 本当はこの関数が失敗したらエラーメッセージを表示するべき。
                (
                    handle,
                    SERVICE_NO_CHANGE,
    ...
    DWORD control(LPCWSTR control)
    {
        ...
        {
            WCHAR buffer[1024];
            wsprintfW(buffer, L"不明なオプション %s が指定されました。", control);
...
namespace service_icon
{
    ...
    HWND create()
    {
        ...
        //  ステータス表示用テキストの準備
        //
        text_table[SERVICE_START_PENDING]     = L"開始中";
        text_table[SERVICE_RUNNING]           = L"開始";
        text_table[SERVICE_PAUSE_PENDING]     = L"一時停止中";
        text_table[SERVICE_PAUSED]            = L"一時停止";
        text_table[SERVICE_CONTINUE_PENDING]  = L"再開中";
        text_table[SERVICE_STOP_PENDING]      = L"停止中";
        text_table[SERVICE_STOPPED]           = L"停止";
...
#pragma warn .8027
    ...
    UINT run(HINSTANCE a_hInstance, HINSTANCE a_hPrevInstance, LPSTR a_lpCmdLine, int a_nCmdShow)
    {
        ...
        else
        {
            ...
            (
                NULL,
                L"タスクバーの通知領域上で監視するサービスを指定してください。\n"
                L"\n"
                L"\n"
